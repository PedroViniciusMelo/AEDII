...
public class GrafoListaDeAdjacencia {
    ...
    public GrafoListaDeAdjacencia(Character[] vertices, Aresta[] arestas) {
        ...
        for (Aresta aresta : arestas) {
            char vertex1 = aresta.aresta.charAt(0);
            char vertex2 = aresta.aresta.charAt(1);

            int indiceVertice1 = this.vertices.indexOf(vertex1);
            int indiceVertice2 = this.vertices.indexOf(vertex2);

            listaDeAdjacencia[indiceVertice1].add(new Aresta(aresta.peso, String.valueOf(vertex2)));
            listaDeAdjacencia[indiceVertice2].add(new Aresta(aresta.peso, String.valueOf(vertex1)));
        }
    }
    ...
    public String toString() {
        ...
        for (int i = 0; i < quantidadeDeVertices; i++) {
            ...
            for (Aresta index : linkedList) {
                graph.append(index.aresta)
                        .append(" ").append("[").append(index.peso).append("]").append('\t')
                ;
            }
            graph.append("\n");
        }
